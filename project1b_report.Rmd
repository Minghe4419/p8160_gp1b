...
run_simulation <- function(n_sim = 1000, n, t, beta0, beta1, sigma, distribution) {
  ...
  for (i in 1:n_sim) {
    ...
    # 2) Fit the naive model and extract metrics from the same model
...
# --- 4. Construct Parameter Grid (including two beta1 values to assess both Type I error and Power) ---
param_grid <- expand.grid(
  n = c(50, 200, 500),                # Sample sizes
  sigma = c(0.5, 1.0, 2.0),             # Random intercept standard deviation (controls correlation strength)
...
for (i in 1:nrow(param_grid)) {
  ...
  # Calculate bias, coverage, and sig_rate
  bias <- mean(sim_res$beta1_est) - params$beta1_val
  coverage <- mean(sim_res$coverage)
  sig_rate <- mean(sim_res$p_value < 0.05)
  
  # Distinguish between Type I error and Power based on beta1_val
  if (params$beta1_val == 0) {
    type1_error <- sig_rate
    power <- NA
  } else {
    type1_error <- NA
    power <- sig_rate
